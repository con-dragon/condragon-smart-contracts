...
contract CompositeNFT is Ownable {
    ...
    function composite(uint256[] calldata _ids, uint256 genesisId) external {
        ...
        if (catId > 0) {
            ...
            userLast[_from] = _lastInfo;
            emit Composite(_from, genesisId, false, failcCount);
        }
    }
    ...
    function _compositeOrFail(
        ...
        if (random < failProbaility[level]) {
            ...
            failcCount++;
            if (luckOnCount[failcCount]) {
                uint256 luckNumber =
                    random
                        .mod(failcCount.sub(LastLuckOnCount))
                        .add(LastLuckOnCount)
                        .add(1);
                emit NewLuckNumber(failcCount, luckNumber);
                luckNumbers.push(luckNumber);
                LastLuckOnCount = failcCount;
            }
        } else {
            if (level > 1) {
