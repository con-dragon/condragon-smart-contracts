...
contract ConDragonStake is Ownable, IERC777Recipient {
    ...
    // weight (place rate * nft cat rate * nft level rate)
    address public cMoonToken;

    // weight config
    // catId => rate
    mapping(uint256 => uint256) public catRates;
    // price => placeRate
    ...
    function setCatRates(uint256[] calldata _catIds, uint256[] calldata _rates) external onlyOwner {
        require(_catIds.length == _rates.length, "Stake: length diff");
        uint256 _range = _catIds.length;

        for(uint256 i = 0; i < _range; i ++){
          catRates[_catIds[i]] = _rates[i];
        }
    }

    function setCatRate(uint256 _catId, uint256 _rate) external onlyOwner {
      catRates[_catId] = _rate;
    }
    ...
    function _stake(address _user, uint256 _placeId, uint256 _tokenId) internal {
      ...
      // record profit
      {
         require(catRates[_catId] > 0, "Stake: catId not rate");
         ...
         uint256 _weight = _userPlace.placeRate.mul(_userPlace.nftLevel).mul(catRates[_catId]);
